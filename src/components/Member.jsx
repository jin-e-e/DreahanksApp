...
const Member = (username) => {
    ...
    async function fetchHolidayInfos() {
        try {
            const holidayInfoData = await API.graphql(graphqlOperation(listHolidayInfos, {
                filter: { userName: { eq: menberUsername } }
            }))
    ...
    return <>
        <ul>
            <Link to={`/about/${menberUsername}`}><Button style={styles.workButton}>勤怠入力</Button></Link>
        </ul>
        ...
        <Button style={styles.button} onClick={handleRegistClickOpen} >登録</Button> <br />
        <Dialog open={registOpen} onClose={handleRegistClose} aria-labelledby="dialogTitle" aria-describedby="dialogDescription">
            ...
            <DialogActions>
                ...
                <Button onClick={handleRegistClose}>いいえ</Button>
            </DialogActions>
        </Dialog>
                            ...
                            {menberUsername === holidayInfo.userName && holidayInfo.confirmFlg === '1' ?
                                [
                                    ...
                                    <TableCell style={{ backgroundColor: 'grey' }}>
                                        <Button variant="outlined" disabled={true} onClick={() => handleDeleteClickOpen(holidayInfo.id)} >削除</Button>
                                ...
                                (menberUsername === holidayInfo.userName && holidayInfo.confirmFlg === '0' ?
                                    [
                                        ...
                                        <TableCell>
                                            <Button variant="outlined" onClick={() => handleDeleteClickOpen(holidayInfo.id)} >削除</Button>
                                            <Dialog open={open} onClose={handleClose} aria-labelledby="alert-dialog-title" aria-describedby="alert-dialog-description">
                                                <DialogTitle id="alert-dialog-title">{"削除"}</DialogTitle>
                                                <DialogContent>
                                                    <DialogContentText id="alert-dialog-description">本当に削除しますか？</DialogContentText>
...
const styles = {
    ...
    button: { backgroundColor: 'black', color: 'white', outline: 'none', fontSize: 15, padding: '12px 0px' },
    workButton: { backgroundColor: 'black', color: 'white', outline: 'none', fontSize: 15 },
